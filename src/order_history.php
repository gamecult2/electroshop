...
if ($view === 'details' && $orderId) {
    ...
    if (!$order || $order['customer_id'] != get_current_user_id()) {
        echo '<div class="container-xxl pb-5"><div class="container-xxl text-center p-5"><h2>' . t('order_not_found') . '</h2><a href="order_history.php" class="btn btn-danger mt-3">' . t('back_to_orders') . '</a></div></div>';
    ...
    $limit = 10;
    $offset = ($page - 1) * $limit;
    $orders = $orderModel->getUserOrders(get_current_user_id(), null);
