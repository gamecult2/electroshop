...
if ($view === 'details' && $orderId) {
    ...
    if ($order && $order['payment_method'] === 'chargily' && $order['payment_status'] !== 'paid' && !empty($order['transaction_id'])) {
        ...
        try {
            ...
            if ($checkout && $checkout->getStatus() === 'paid') {
                ...
                $orderModel->updateStatus($orderId, 'processing', null, 'Payment verified automatically on user view');
...
<style>
    ...
    .card-header.bg-white { border-bottom: 1px solid rgba(0,0,0,.05); }
</style>
